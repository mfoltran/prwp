...
function wppb_select2_multiple_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){
	if ( $field['field'] == 'Select2 (Multiple)' ){
		...
		wp_enqueue_script( 'wppb_sl2_js', WPPBSL2_PLUGIN_URL . 'assets/js/wppb-select2-front-end.js', array( 'jquery' ) );
        ...
        $input_value = ( isset( $request_data[wppb_sl2_multiple_handle_meta_name( $field['meta-name'] )] ) ? array_map( 'esc_attr', $request_data[wppb_sl2_multiple_handle_meta_name( $field['meta-name'] )] ) : $input_value );
		...
		if ( $form_location != 'back_end' ){
			...
			$output = '
				...
				<select name="'.esc_attr( $field['meta-name'] ).'[]" id="'.esc_attr( wppb_sl2_multiple_handle_meta_name( $field['meta-name'] ) ).'" class="custom_field_select2 custom_field_multiple_select2" multiple="multiple" '. $extra_attr .' data-wppb-select2-arguments=\'' .  json_encode($arguments) . '\'>';
			...
			$output = '
				<table class="form-table wppb-select2">
					<tr>
						...
						<td>
							<select name="'. esc_attr( $field['meta-name'] ) .'[]" class="custom_field_select2" id="'.esc_attr( wppb_sl2_multiple_handle_meta_name( $field['meta-name'] ) ).'" multiple="multiple" '. $extra_attr .' data-wppb-select2-arguments=\'' . json_encode($arguments)  . '\'>';
 ...
 * @return string
 */
function wppb_select2_multiple_enable_tags($field_options, $form_location, $field, $user_id, $field_check_errors, $request_data ){
	...
	$merged_arrays = array_merge($existing_values_array, $user_values_array);
