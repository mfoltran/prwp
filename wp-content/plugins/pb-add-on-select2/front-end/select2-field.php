...
function wppb_select2_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){
	if ( $field['field'] == 'Select2' ){
        ...
        $input_value = ( isset( $request_data[wppb_sl2_handle_meta_name( $field['meta-name'] )] ) ? esc_attr( stripslashes( trim( $request_data[wppb_sl2_handle_meta_name( $field['meta-name'] )] ) ) ) : $input_value );
...
function wppb_save_select2_value( $field, $user_id, $request_data, $form_location ){
	if( $field['field'] == 'Select2' ){
		if ( isset( $request_data[wppb_sl2_handle_meta_name( $field['meta-name'] )] ) )
			update_user_meta( $user_id, sanitize_text_field( $field['meta-name'] ), sanitize_text_field( $request_data[wppb_sl2_handle_meta_name( $field['meta-name'] )] ) );
	}
}
